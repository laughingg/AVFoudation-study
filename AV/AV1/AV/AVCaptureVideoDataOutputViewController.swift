...
//        }
        ...
        let supportedFrameRateRanges = inputDevice?.activeFormat.videoSupportedFrameRateRanges
...
//            if CMTIME_COMPARE_INLINE() && CMTIME_COMPARE_INLINE() {
//            
//            }
            
            if CMTimeCompare(frameDuration, (range as AnyObject).maxFrameDuration) <= 0 &&
                CMTimeCompare(frameDuration,  (range as AnyObject).minFrameDuration) >= 0 {
            
                frameRateSupported  = true
            }
            
            print(CMTimeCompare(frameDuration, (range as AnyObject).maxFrameDuration) <= 0)
            print(CMTimeCompare(frameDuration,  (range as AnyObject).minFrameDuration) >= 0)
        }
        
        print(supportedFrameRateRanges)
        
        if frameRateSupported {
//            && inputDevice?.lockForConfiguration()
        
            do {
                try inputDevice?.lockForConfiguration()
                
                inputDevice?.activeVideoMaxFrameDuration = frameDuration
                inputDevice?.activeVideoMinFrameDuration = frameDuration
